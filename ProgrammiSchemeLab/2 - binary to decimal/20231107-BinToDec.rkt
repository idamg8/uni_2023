...
(define base-val-dec
  (lambda (b s)
    (if (= (string-length s) 1)
        (* (expt (lun-base b) -1) (valore-cifra b (string-ref s 0)))
        (+
         (* (expt (lun-base b) -1) (base-val-dec b (substring s 1)))
         (* (expt (lun-base b) -1) (valore-cifra b (string-ref s 0)))
         )
        )
    ))

(define base-to-dec  ; val: numero razionale in base 10 di valore corrispondente alla stringa 
  (lambda (b s)   ; s: stringa (eventualmente decimale e/o con segno)
    (cond ((string=? s "")
           "")
          ((char=? (string-ref s 0) #\-)
           (if (eqv? (punto s) "")
           (* -1 (base-val b (substring s 1)))
           (* -1 (+ (base-val b (substring s 1 (- (punto s) 1))) (base-val-dec b (substring s (punto s)))))
           ))
          ((char=? (string-ref s 0) #\+)
           (if (eqv? (punto s) "")
           (base-val b (substring s 1))
           (+ (base-val b (substring s 1 (- (punto s) 1))) (base-val-dec b (substring s (punto s))))
           ))
          ((string=? s ".")
           ".")
          ((char=? (string-ref s 0) #\.)
           (base-val-dec b (substring s 1))
          )
          (else
           (if (eqv? (punto s) "")
           (base-val b s)
           (+ (base-val b (substring s 0 (- (punto s) 1))) (base-val-dec b (substring s (punto s))))
           ))
          )
    ))