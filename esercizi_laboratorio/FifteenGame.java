...
public class FifteenGame {
    ...
    public void moveTile(int n) { 
        // trovo le coordinate di n (tassello da spostare)
        int[] coordsN = findCoords(n);
        int i = coordsN[0];
        int j = coordsN[1];
        
        // trovo le coordinate di 0)
        int[] coordsZ = findCoords(0);
        int x = coordsZ[0];
        int y = coordsZ[1];

        // sposto n e 0 all'interno dell'array numList (usando i*n+j)
        numList[i*size+j] = 0;
        numList[x*size+y] = n;
    ...
    public static void playGame(int n) {
        ...
        for (int i=1; i<=n; i++) {
            for (int j=1; j<=n; j++) {
                gui.setNumber(i,j,t.tavoletta[i-1][j-1]);  // -1 perché setNumber ha indici che partono da 1 mentre l'array parte da 0
            }
        }

        while (!t.isOrdered()) {  // procedo finché la tavoletta non sarà ordinata

            // prendo numero che l'utente ha cliccato
            int k = gui.get();

            if (t.isMovable(k)) {  // controllo se la casella cliccata è spostabile
                ...
                gui.clear(i+1,j+1);  // con clear svuoto una casella (diventa 0)
